...
\begin{itemize}[nosep]
    \item \textbf{Agentic 评测与优化框架:} 参与构建基于 LLM-as-a-Judge 的 Microsoft Excel Agent 闭环评测与优化框架。整合模型输出与执行轨迹进行自动化缺陷挖掘、根因诊断及多轮实验验证，实现Agent问题细粒度诊断与优化自动化。
    \item \textbf{Agent 检索与工具优化：} 设计并落地生产环境的 Agent 检索优化，包括邮件线程上下文按需加载及组织关系查询的批量图遍历。降低冗余上下文暴露与工具调用开销并提升检索覆盖率。
\end{itemize}
